...
- **Скачивание резюме** — кнопка с лоадером и блокировкой: спиннер → галочка ✓ →
  снекбар 10с (кольцо-таймер с обратным отсчётом, кнопка «Открыть», закрытие ✕)
- **Тёмная тема** — переключатель в шапке (☀/☾), сохраняется в localStorage,
  без «вспышки» при загрузке (ранний скрипт в index.html), дефолт — система
- **i18n/SEO** — ru/zh/en/de, динамические lang/title/description, JSON-LD Person,
  OG/Twitter-карточка, sitemap.xml, robots.txt
- **PDF-резюме** — скачивается на текущем языке; вёрстка A4 с воздухом между
  блоками (вертикальный ритм по БП резюме-вёрстки)
